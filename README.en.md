...
[![Python 3.9+](https://img.shields.io/badge/Python-3.9%2B-3776AB?style=flat-square&logo=python&logoColor=white)](README.en.md)
...
<sub>The UI ships in **Chinese and English** — click the `中文 / English` button in the top
bar, or open with `?lang=en`. The screenshot uses demo data from the screenshot script
(fake keys, a local fake balance endpoint) — it is nobody's real account.</sub>

## Install — pick one of three

**1. macOS: just download it (recommended)**

[**⬇︎ Download the full app**](https://github.com/yankuanglin1-droid/codex-model-switcher/releases/latest/download/Codex-Model-Switcher-macOS-latest-full.zip)
— ~52 MB, **bundles its own Python**: no Homebrew, no Xcode command line tools. Unzip and
double-click. Already have Python 3.9+? The
...
> Those two URLs are **permanent aliases** — they always point at the newest release.
> First launch: right-click the app → **Open**, once.

**2. One command (macOS / Linux)**
...
Paste the prompt from [INSTALL-WITH-AI.md](INSTALL-WITH-AI.md) into Codex / Claude Code.
It asks for the provider and the key, installs, configures, verifies, and tells you how to switch.
...
- **Bilingual UI** — one click in the top bar toggles 中文 / English, and the choice is
  remembered; `?lang=en` opens straight into English
